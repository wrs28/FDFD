...
"""
ψ, A, inputs = scatter(inputs, k; isNonLinear=false, dispOpt=false, ψ_init=[],
...
"""
function scatter(inputs::InputStruct, k::Union{Float64,Int};
    ...
    ψ, A, inputs = scatter(inputs, complex(1.0*k); isNonLinear=isNonLinear, dispOpt=dispOpt,
        ψ_init=ψ_init, F=F, truncate=truncate, A=A, fileName=fileName,
        ftol=ftol, iter=iter)
end
function scatter(inputs::InputStruct, k::Complex128;
    isNonLinear::Bool=false, dispOpt::Bool = false, ψ_init::Array{Complex128,1}=Complex128[],
    F::Array{Float64,1}=[1.], truncate::Bool=false,
    A::Base.SparseArrays.UMFPACK.UmfpackLU=lufact(speye(1,1)), fileName::String = "",
    ftol::Float64=2e-8, iter::Int=150)::
    ...
    if !isNonLinear
        ψ, A, inputs = scatterL(inputs, k; A=A, F=F)
    elseif isNonLinear
        ψ, A, inputs = scatterNL(inputs, k; A=A, F=F, dispOpt=dispOpt, ψ_init=ψ_init, ftol=ftol, iter=iter)
    end
...
end # end of function scatter


"""
ψ, A, inputs = scatterL(inputs, k; A=[], F=[1.])
    ...
    A is a factorized wave operator.
"""
function scatterL(inputs::InputStruct, k::Complex128;
    ...
    j, ∇² = synthesize_source(inputs1, k)

    if (A.m == A.n == 1)
        N = prod(inputs1.N_ext); ε_sm = inputs1.ε_sm; F_sm = inputs1.F_sm
        ɛk² = sparse(1:N, 1:N, ɛ_sm[:]*k², N, N, +)
        χk² = sparse(1:N, 1:N, inputs1.D₀*γ(k,inputs1)*F.*F_sm[:]*k², N, N, +)
        A = factorize(∇²+ɛk²+χk²)
    end
    ...
    return ψ, A, inputs1
end # end of function scatterL


"""
ψ, ϕ, A, inputs = scatterNL(inputs, k; dispOpt=false, ψ_init=[],
    ...
    A is a factorized wave operator.
"""
function scatterNL(inputs1::InputStruct, k::Complex128;
...
"""
S =  smatrix(inputs, k; isNonLinear=false, F=[1.], dispOpt = true,
                                    N=1, N_Type="D", ψ_init = [], fileName = "")

    N is the number of steps to go from D0 = 0 to given D0
"""
function smatrix(inputs::InputStruct, k::Union{Complex128,Float64,Int};
    channels::Array{Int,1}=Array(1:length(inputs.channels)),
    ...
    K = [complex(1.0*k)]
    S = smatrix(inputs, K; channels=channels, isNonLinear=isNonLinear, F=F,
                dispOpt=dispOpt, fileName=fileName, N=N, N_type=N_type,
                ψ_init=ψ_init, parallel=parallel)

    return S
end # end of fuction computeS
function smatrix(inputs::InputStruct, k::Array{Float64,1};
    ...
    S = smatrix(inputs, complex(k); channels=channels, isNonLinear=isNonLinear, F=F,
                    dispOpt=dispOpt, fileName=fileName, N=N, N_Type=N_tyoe,
                    ψ_init=ψ_init, parallel=parallel)

    return S
end # end of fuction computeS
function smatrix(inputs::InputStruct, k::Array{Complex128,1};
    channels::Array{Int,1}=Array(1:length(inputs.channels)), isNonLinear::Bool=false,
    F::Array{Float64,1}=[1.], dispOpt::Bool=true, fileName::String = "",
    N::Int=1, N_Type::String="D", ψ_init::Array{Complex128,1}=Complex128[],
    parallel::Bool=false)::Array{Complex128,4}
    ...
    if !isNonLinear & parallel
        S = smatrixL(inputs, k, parallel; channels=channels, F=F, dispOpt=dispOpt,
                        fileName=fileName)
    elseif !isNonLinear & !parallel
        S = smatrixL(inputs, k; channels=channels, F=F, dispOpt=dispOpt,
                        fileName=fileName)
    elseif isNonLinear & parallel
        S = smatrixNL(inputs, k, parallel; channels=channels, N=N, N_Type=N_Type,
                        isNonLinear=isNonLinear, F=F, dispOpt=dispOpt,
                        ψ_init=ψ_init, fileName=fileName)
    elseif isNonLinear & !parallel
        S = smatrixNL(inputs, k; channels=channels, N=N, N_Type=N_Type,
                        isNonLinear=isNonLinear, F=F, dispOpt=dispOpt,
                        ψ_init=ψ_init, fileName=fileName)
    end
...
"""
S =  smatrixL(inputs; F=[1.], dispOpt=true, fileName = "")
"""
function smatrixL(inputs::InputStruct, k::Array{Complex128,1};
    ...
    for ii in 1:nk
        ...
        for m in 1:nc
            ...
            ψ, ζ, inputs_s = scatterL(inputs, k[ii]; A=ζ)
            for m′ in 1:M
...
S =  computeS(inputs::InputStruct; N=10, N_Type="D", isNonLinear=false, F=1.,
    ...
    N is the number of steps to go from D0 = 0 to given D0
"""
function smatrixNL(inputs1::InputStruct, k::Array{Complex128,1};
